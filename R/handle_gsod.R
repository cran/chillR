...
#' 
#' @export handle_gsod
handle_gsod <- function(action,                      ##### can now also be set to 'delete' and work with clean_up (see below) to delete data
                        location = NULL,
                        ...
                        override_confirm_delete=FALSE, ### in combination with 'action = delete', this overrides the need for user confirmation
  ...
  list_weather_stations <- function(time_interval=time_interval, 
    ...
    stat_list <- 
      ...
      rename(Long = contains("lo"),
             ...
             Elev = contains("ele"))
    stat_list <- stat_list %>% 
      dplyr::mutate(chillR_code = paste(stat_list$USAF, stat_list$WBAN, sep = "")) %>% 
    ...
    stat_list_filtered <-
      stat_list %>% 
      dplyr::mutate(Distance = round(sp::spDistsN1(as.matrix(stat_list[c("Long", "Lat")]),
    ...
    stat_list_filtered <- 
      ...
      dplyr::mutate(Overlap_years = round(((overlap_desctools(c(start_date,end_date),
                                                              c(as.Date(as.character(.data$BEGIN),format="%Y%m%d"),as.Date(as.character(.data$END),format="%Y%m%d")))
                                            + 1) / 365.25), 2),
                    Perc_interval_covered = round(((overlap_desctools(c(start_date,end_date),
                                                                      c(as.Date(as.character(.data$BEGIN),format="%Y%m%d"),as.Date(as.character(.data$END),format="%Y%m%d")))
                                                    + 1) / 365.25 / amount_of_years * 100), 0)) %>% 
      dplyr::ungroup() %>% 
      dplyr::filter(.data$Perc_interval_covered>=min_overlap) %>% 
  ...
  download_weather_stations <- function(time_interval,
    ...
    time_link <- rep(raw_link, times = amount_of_years) %>%
      paste0(years,"/")
    location <- sapply(location, function (x) str_replace(x,"_",""))
    ...
    for (i in 1:length(location)) {
      ...
      names(dataset_links)[i] <-
        dplyr::filter(stat_list, .data$chillR_code == names(subsets_list)[i])$STATION.NAME
    }
      ...
      for (i in 1:length(location)) {
        ...
        lapply(dataset_links[[i]],
               function(link){
                 ...
                 station_id <- stringr::str_split_i(link, "/",-1) %>% stringr::str_split_i(".csv",1)
    ...
    multimerge <- function(){
      ...
      subfolders <- subfolders[stringr::str_extract(subfolders, "\\d+") %in% location]
      all_data <- list()
  ...
  clean_downloaded_weather <- function(action, add.DATE = TRUE){
    ...
    clean_up <- function(data) {
      if (nrow(as.data.frame(data)[1]) == 28) {
        ...
        if("weather" %in% names(data)) old_format <- TRUE else old_format <- FALSE
        if(old_format) dat <- data$weather else dat <- data
        temp_data_complete <-
          data.frame(
            Date = as.Date(dat$DATE),
            Year = format(as.Date(dat$DATE), "%Y") %>% as.numeric(),
            Month = format(as.Date(dat$DATE), "%m") %>%  as.numeric(),
            Day = format(as.Date(dat$DATE), "%d") %>% as.numeric(),
            Tmin = ifelse(dat$MIN == 9999.9,NA,round( ((dat$MIN - 32) * 5/9), 3)),
            Tmax = ifelse(dat$MAX == 9999.9,NA,round( ((dat$MAX - 32) * 5/9), 3)),
            Tmean = ifelse(dat$TEMP == 9999.9,NA,round( ((dat$TEMP - 32) * 5/9), 3)),
            Prec = ifelse(dat$PRCP == 99.99,NA,round((dat$PRCP*25.4),3))
          )
        if(old_format) temp_data_complete<-list(database="GSOD",weather=temp_data_complete)
      }
      temp_data_complete
    }
    ...
    temp_data_complete <- lapply(temp_data_incomplete, function(tab){
      #empty case
      if("weather" %in% names(tab)) old_format <- TRUE else old_format <- FALSE
      if(old_format) tab <- tab$weather
      if(nrow(tab) == 1){
        out<-data.frame(Date = NA,Year = NA,Month = NA,Day = NA,
                   Tmin = NA,Tmax = NA,Tmean = NA,Prec = NA)
      } else {
      ...
      if(old_format) out<-list(database="GSOD",weather=out)
      out
    })
  ...
  ### now for the main body of the function
